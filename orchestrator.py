"""
AI Employee — Orchestrator (Decision Layer) [Silver Tier]
...
LINKEDIN_KEYWORDS = {"post", "publish", "linkedin", "tweet", "social media", "social"}
...
def detect_skill(text: str) -> str | None:
    ...
    Returns 'gmail-send', 'linkedin-post', or None.
    """
    lower = text.lower()

    # Check frontmatter action field first (explicit skill binding)
    fm, body = parse_frontmatter(text)
    ...
    # Fall back to keyword detection on body
    body_lower = body.lower()
    gmail_hits = sum(1 for kw in GMAIL_KEYWORDS if re.search(rf"\b{kw}\b", body_lower))
    linkedin_hits = sum(1 for kw in LINKEDIN_KEYWORDS if re.search(rf"\b{kw}\b", body_lower))

    if gmail_hits > linkedin_hits and gmail_hits > 0:
        return "gmail-send"
    if linkedin_hits > gmail_hits and linkedin_hits > 0:
        return "linkedin-post"
    if gmail_hits > 0:
        return "gmail-send"
...
def run_skill(skill_name: str, task_id: str) -> tuple[bool, str]:
    ...
    """
    if skill_name == "gmail-send":
        script = GMAIL_SEND_SCRIPT
    elif skill_name == "linkedin-post":
        script = LINKEDIN_POST_SCRIPT
    else:
        return False, f"Unknown skill: {skill_name}"

    if not script.exists():
        return False, f"Skill script not found: {script}"
...
def triage_task(path: Path) -> bool:
    ...
    # ── No approval needed — write back and stay in Needs_Action ─────
    try:
        rewrite_task_file(path, fm, body)
    except Exception as exc:
        stats["errors"] += 1
        log_action("write_error", path.name, str(exc))
        return False

    stats["tasks_triaged"] += 1
    plan_note = f" + {plan_ref}" if plan_ref else ""
    stats["last_event"] = f"[green]✓ Triaged {path.name} → {category}{plan_note}[/]"
    stats["recent_actions"].append({
        "action": f"Triaged {path.name}",
        "detail": f"→ {category} ({priority}){plan_note}",
        "time": datetime.now(timezone.utc).strftime("%H:%M:%S"),
    })
    stats["recent_actions"] = stats["recent_actions"][-10:]

    log_action("triage", path.name, "success", category=category, priority=priority,
               plan_file=plan_ref)
    return True
...
def main() -> None:
    ...
    console.print(
        Panel.fit(
            "[bold green]AI EMPLOYEE — ORCHESTRATOR[/]\n"
            "[dim]Decision Layer v0.1  •  Local-First  •  Autonomous[/]\n\n"
